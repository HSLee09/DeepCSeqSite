...
class SeqDataSet:
	...
	def __init__(self, featureSource, labelSource):
		self.feature = self.ReadFeatureSource(featureSource) # .feat file read
	...
	def Fill(self, src_seqs, filler):
		...
		for i in range(0, batch_size):
			...
			dest_seqs.append(src_seqs[i] + [filler for j in range(pad_size)])
	...
	def NextBatch(self, batch_size = 1):
		...
		if(self.index + batch_size <= self.number):
			feature_batch = self.feature[self.index: self.index + batch_size]
			label_batch = self.label[self.index: self.index + batch_size]
		...
		feature_batch = self.feature[self.index:] + self.feature[:next_end]
		label_batch = self.label[self.index:] + self.label[:next_end]
		self.index = next_end
	...
	def NextRestrictedPaddingBatch_original(self, batch_size = 1):
		if(self.avail_num > 0):
	...
	def NextRestrictedPaddingBatch(self, batch_size=1):
		if (self.avail_num > 0):
			if self.avail_num >= batch_size:
				batch_size = min(self.avail_num, batch_size)
				self.avail_num -= batch_size
				return self.NextPaddingBatch(batch_size)
			else:
				last_batch = self.avail_num
				self.avail_num = 0
				batch_x, batch_y, len_x, len_y = self.NextPaddingBatch(last_batch)
				while len(batch_x) != batch_size:
					batch_x.append([0.] * len_x[0])
					batch_y.append([[1.0, 0.0]] * len_y[0])
					len_x.append(0)
					len_y.append(0)
				return batch_x, batch_y, len_x, len_y
